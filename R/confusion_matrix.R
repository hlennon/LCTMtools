...
#' library(lcmm)
#' model1 <- lcmm::hlme(BMI ~Age, mixture= ~Age, random= ~Age, nwg=TRUE, ng=2, subject="ID", data=bmi_long[1:500, ])
#' model2 <- lcmm::hlme(BMI ~Age, mixture= ~Age, random= ~1, nwg=FALSE, ng=2, subject="ID", data=bmi_long[1:500, ])
